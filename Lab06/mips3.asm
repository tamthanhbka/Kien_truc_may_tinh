...
sort:	beq $s0, $zero, after_sort # neu khong co su thay doi, hoan thanh so sanh
		nop
		j max # co su thay doi -> nhay den max de thuc hien so sanh
		nop
after_max:	addi $s0, $s0, 1
			...
			j set
			nop		
max:		addi $s0, $zero, 0 # set so lan thay doi ve 0
		addi $t0, $zero, 0 # i = 0
...
loop:	slt $t3, $t0, $a1 # i < n ?
		...
		slt $t3, $t2, $t1 # neu phan tu ke tiep nho hon phan tu hien tai
		bne $t3, $zero, after_max
		nop
set:		addi $t0, $t0, 1
		add $s2, $t0, $t0  #$s2 = 2i
		add $s2, $s2, $s2  # $s2 = 4i
